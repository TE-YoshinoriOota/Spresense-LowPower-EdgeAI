/*
 *  Spresense_BMI160_recognition.ino - Motion recognition sample
...
#include <BMI160Gen.h>
#include <Flash.h> 
#include <float.h>  // floatの最大値・最小値の定義
#include <DNNRT.h>
DNNRT dnnrt;

// センサー出力更新周期: 25, 50, 100, 200, 400, 800, 1600 (Hz)
const int sense_rate = 200;
// ジャイロセンサ範囲: 125, 250, 500, 1000, 2000 (deg/sec)
...
// センサー出力値が16bit整数(±32768)のため浮動小数点に変換
...
void setup() {
  ...
  // フラッシュROMにある学習済モデルをオープン
  File nnbfile = Flash.open("model.nnb");
  if (!nnbfile) {
    Serial.println("model.nnb not found");
    return;
  } 
  // DNNRTライブラリを学習済モデルで開始
  int ret = dnnrt.begin(nnbfile);
  if (ret < 0) {
    Serial.println("DNNRT error: " + String(ret));
    nnbfile.close();
    return;
  }
...
void loop() {
  ...
  if ((abs(rpm_roll[point-2])  < threshold 
   ...
   || (abs(rpm_pitch[point-2]) < threshold 
    ...
    DNNVariable input(sense_rate);  
    float* data = input.data();
    float range = abs(pmax) > abs(pmin) ? 
                  abs(pmax) : abs(pmin);
    for (int n = 0; n < sense_rate; ++n) {
      data[n] = rpm_pitch[n]/range;
    }
    // DNNRTで推論を実行
    ...
    if (output[index] > 0.9) {
      switch(index) {
      case 0: digitalWrite(LED0, HIGH); break; //ダブルタップ
      case 1: digitalWrite(LED1, HIGH); break; //シングルツイスト
      case 2: digitalWrite(LED2, HIGH); break; //ダブルツイスト
      } 
    }
