/*
 *  MainAudioHST.ino - Sample of producing sound pressure histogram using FFT
...
void loop() {
  ...
  // ヒストグラムをシフト
  for (int t = 1;  t < frames; ++t)  hist[t-1] = hist[t];
  ...
  if (pre_area < silent_th 
    && target_area >= sound_th && post_area < silent_th) {
    for (int t = 0; t < frames; ++t) {
      Serial.println(String(t) + "," + String(hist[t]));
    }
